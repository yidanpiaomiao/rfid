...
//add by wwh
static void print(const char *name,Block buff){
	 printf("  %s = 0x", name);
...
static void test_elligator() {
    //decaf::SpongeRng rng(decaf::Block("test_elligator"));
	   decaf::SpongeRng rng(decaf::Block("caonima"));
    Test test("Elligator");
    ...
    for (int i=0; i<NTESTS && test.passing_now; i++) {
        ...
        Point s;
        //add by wwh
//        printf("b1:%s",b1.get_string().c_str());
//        printf("b2:%s",b2.get_string().c_str());
      //  print("b1",b1);
        ...
        //caution by wwh
      //  print("wwh-point", t);
        SecureBuffer sb=t.steg_encode(rng);
      //  printf("wwh-encode%s",sb.get_string().c_str());
        Point result=Point::from_hash(sb);
       // print("wwh-result", result);

        //test wrong!
//        SecureBuffer buff=SecureBuffer(Point::HASH_BYTES);
//        unsigned char one=0x01;
//        result.invert_elligator(buff,one);
//        cout<<"buff"<<buff.get_string()<<endl;

        //test wrong
//        unsigned char buff[56];
//    	result.encode(buff);
//    	cout<<"test buff"<<buff<<endl;

        //test wrong
//        Point de;
//        decaf_bool_t flag=Point::decode(de,sb);
//        cout<<"suc?"<<flag<<endl;
//        print("de",de);

        point_check(test,t,t,t,0,0,t,result,"steg round-trip");
    }
}
...
//        printf(p==q?"true":"false");
        ...
        //add by wwh
//        Point wwh=Point::from_hash(buffer);
//        print("r",r);
//        print("wwh",wwh);
//        printf(r==wwh?"true":"false");
...
//add by wwh
//test 1KB 10KB 100KB 1MB 10MB consume time
...
//	double all_dec_time = 0;
	int cycle = 1;
	const uint32_t message_size = 1024*1024*10;
